...
function formatSnapshotForPrompt(snapshot) {
  ...
  const lines = [
    ...
    '',
    '── FINANCIAL STATEMENT ──',
    `Opening Balance:  ${fmt(snapshot.summary.openingBalance)}  ← carried forward from prior periods`,
    `Total Income:     ${fmt(snapshot.summary.totalIn)}`,
    `Total Expenses:   ${fmt(snapshot.summary.totalOut)}`,
    `Net This Period:  ${fmt(snapshot.summary.net)}`,
    `Closing Balance:  ${fmt(snapshot.summary.closingBalance)}  ← true available position`,
